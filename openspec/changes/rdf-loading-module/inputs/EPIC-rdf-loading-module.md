...
<!-- Written during planning. Do not modify during implementation. -->

## Description

The RDF Loading Module replaces the legacy shell script `resources/load_versions.sh`
(invoked via `subprocess` from `rdf_differ/adapters/skos_history_wrapper.py`) with a
clean, layered, fully testable Python implementation.

The module loads two or more complete RDF vocabulary versions as named graphs, computes
triple-level **insertions** and **deletions** between version pairs (the *skos-history*
four-graph delta pattern), and materialises the version-history metadata that downstream
tools (`diff-query-generator`-produced queries, the report builder) rely on.

The single most important new capability is **dual-mode operation**:

1. **Remote mode** — load and diff against an existing SPARQL 1.1 triple store
   (Apache Jena Fuseki today), preserving current production behaviour.
2. **In-memory mode** — load and diff entirely in-process (no triple store required),
   exposed through a CLI, for temporary / ephemeral / CI / local diffs.

Both modes execute the **same** delta-computation logic against the **same** named-graph
contract, selected by dependency injection behind a single port interface.

This EPIC owns **version loading + low-level delta-graph construction + version-history
metadata**. It does **not** own higher-level change-category reporting — that remains the
responsibility of the `diff-query-generator`-produced SPARQL queries and the existing report
builder. The contract between the two is the four named graphs plus the version-history graph
(see Glossary).
...
- **Tests first**: unit tests per layer + BDD features (this Epic ships the Gherkin separately).
...
| Put report/change-category logic here | Leave it to dqgen queries + report builder | Preserve module boundary |
...
1. **Domain models** — `VersionStoreConfig`, `VersionSpec`, `UriBuilder`, `BlankNodePolicy`,
   delta-pair functions. *Layers:* models. *Deps:* none. *AC:* T1, T2, T4 pass; no framework imports.
2. **`GraphStorePort` + query templates** — port interface; SPARQL templates/prefixes as constants.
   *Layers:* adapters (+ models for value objects). *Deps:* 1. *AC:* templates parametrised, no free strings.
3. **In-memory adapter** — `pyoxigraph.Store` implementation (load/update/serialize/clear/ask).
   *Layers:* adapters. *Deps:* 2. *AC:* T3, T7 pass in-memory; add `pyoxigraph` dep.
4. **Remote adapter** — HTTP GSP + SPARQL Update/Query (reuse `requests`/`SPARQLWrapper`).
   *Layers:* adapters. *Deps:* 2. *AC:* T8 pass (mocked).
5. **Loading service** — orchestrates the flow; depends only on `GraphStorePort`.
   *Layers:* services. *Deps:* 1–2. *AC:* T5, T6 pass.
6. **Validation service** — structural + content `ASK` checks (spec §10).
   *Layers:* services. *Deps:* 5. *AC:* T7 pass.
7. **CLI entrypoint** — `click` command; mode flag; YAML config. *Layers:* entrypoints. *Deps:* 5–6. *AC:* in-memory CLI happy path.
8. **Integrate & retire script** — route `FusekiDiffAdapter.create_diff` to the remote service;
   delete `skos_history_wrapper.py` subprocess path + `load_versions.sh`. *Layers:* adapters/services.
   *Deps:* 4–6. *AC:* existing API/Celery diff flow green; old BDD specs adapted.
9. **importlinter contracts** — enforce layer direction + store-seam isolation. *Deps:* 1–7.

## Roadmap
- [ ] Task 1: Domain models
- [ ] Task 2: GraphStorePort + query templates
- [ ] Task 3: In-memory adapter (pyoxigraph)
- [ ] Task 4: Remote adapter (Fuseki GSP + Update)
- [ ] Task 5: Loading service
- [ ] Task 6: Validation service
- [ ] Task 7: CLI entrypoint
- [ ] Task 8: Integrate dual-mode & retire load_versions.sh
- [ ] Task 9: importlinter contracts
...
- Spec: `docs/spec/rewriting-sh-to-python/delta_graphs_loading_and_computation_spec.md` §7 (delta rules), §8 (process), §10 (validation)
- Synthesis: `docs/spec/rewriting-sh-to-python/on-using-version-graphs-synthesis.md`
- Implementation plan: `docs/spec/rewriting-sh-to-python/IMPLEMENTATION-PLAN-rdf-loading-module.md`
- Gherkin: `docs/spec/rewriting-sh-to-python/rdf_loading_module.feature`
...
<!-- One dated entry per task during implementation. -->

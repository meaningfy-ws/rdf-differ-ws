...
from rdf_differ.api.entrypoints.ui.api_wrapper import (
    build_report,
...
from rdf_differ.api.entrypoints.ui.api_wrapper import create_diff as api_create_diff
from rdf_differ.api.entrypoints.ui.api_wrapper import get_active_tasks as api_get_active_tasks
from rdf_differ.api.entrypoints.ui.api_wrapper import revoke_task as api_revoke_task
from rdf_differ.api.entrypoints.ui.forms import BuildReportForm, CreateDiffForm
from rdf_differ.api.entrypoints.ui.helpers import get_error_message_from_response
...
def view_dataset(dataset_id: str):
    ...
    try:
        form.application_profile.choices = [
            (item["application_profile"], item["application_profile"])
            for item in application_profiles
        ]
        form.template_type.choices = [
            (item, item) for item in application_profiles[0]["template_variations"]
        ]
...
def download_report(dataset_id: str, application_profile: str, template_type: str):
    ...
    try:
        with tempfile.TemporaryDirectory() as temp_folder:
            report_content, extension, _ = get_report(
                dataset_id, application_profile, template_type
            )

...
class CreateDiffForm(FlaskForm):
    ...
    dataset_description = StringField(
        "Dataset description", description="Optional description of the dataset."
    )
    ...
    old_version_id = StringField(
        "Old dataset version name", validators=[DataRequired()], default="old"
    )
    new_version_file_content = FileField("New dataset file", validators=[FileRequired()])
    new_version_id = StringField(
        "New dataset version name", validators=[DataRequired()], default="new"
    )

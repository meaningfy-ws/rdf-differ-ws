...
def create_diff(
    body: dict, old_version_file_content: FileStorage, new_version_file_content: FileStorage
) -> tuple:
    """
    ...
    if can_create:
        try:
            with save_files(
                old_version_file_content, new_version_file_content, config.RDF_DIFFER_FILE_DB
            ) as (db_location, old_version_file, new_version_file):
...
def get_task_status(task_id: str) -> tuple:
    ...
    if task:
        ...
        data = {"task_id": task.id, "status": task.status, "result": result}
        return data, 200

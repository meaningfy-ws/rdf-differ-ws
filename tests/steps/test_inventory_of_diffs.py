...
def fuseki_diff_getter():
    ...
    return FusekiDiffGetter("http://localhost:3030")
...
def the_dataset_uri_is_returned(diff_object):
    ...
    assert diff_object[0]['datasetURI'] == 'http://publications.europa.eu/resource/authority/subdivision'
...
def at_least_two_dataset_versions_are_returned(diff_object):
    ...
    assert len(diff_object[0]['datasetVersions']) == 2

...
def files(tmpdir):
    ...
    shutil.copy(
        Path("tests/test_data/original/subdivisions_sh_ds/data/v1/subdivisions-skos.rdf"),
        old_version_file,
    )
    new_version_file = tmpdir.join("new_version.rdf")
    shutil.copy(
        Path("tests/test_data/original/subdivisions_sh_ds/data/v2/subdivisions-skos.rdf"),
        new_version_file,
    )
...
@scenario(
    "../features/diffing_two_dataset_versions.feature",
...
def the_diff_calculator_is_executed():
    ...
    assert helper_fuseki_service(
        http_client=requests, sparql_client=SPARQLRunner()

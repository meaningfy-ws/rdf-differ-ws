...
def test_list_command():
    result = subprocess.run(
        [SCRIPT, "--base-url", BASE_URL, "list"], capture_output=True, text=True
    )
    assert result.returncode == 0, f"List command failed: {result.stderr}\n{result.stdout}"
    assert "Response" in result.stdout or "Raw response" in result.stdout, (
        "List output missing expected response"
    )

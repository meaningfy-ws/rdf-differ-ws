...
def test_get_diffs_200(mock_list_datasets, mock_dataset_description):
    ...
    assert status == 200
    assert response == [{"dataset_id": "first_dataset"}, {"dataset_id": "second_dataset"}]
...
def test_create_diff_200_empty_dataset(_, mock_init, mock_dataset_description):
    ...
    response, status = create_diff(
        body=body, old_version_file_content=file_1, new_version_file_content=file_2
    )
...
def test_get_diff_200(
    ...
    mock_build_dataset_reports_location,
    mock_read_meta_file,
):
    dataset_name = "dataset"

    mock_dataset_description.return_value = {"dataset_name": "dataset"}
    mock_read_meta_file.return_value = {"uid": "1", "dataset_name": dataset_name}
...
def test_build_report_200(
    ...
    response, status = build_report(
        {"dataset_id": "dataset", "application_profile": "ap", "template_type": "tt"}
    )
...
def test_build_report_406_exists(
    ...
    assert (
        "Report already exists. To rebuild send the `rebuild` query parameter set to true"
        in response["message"]
    )
    assert status == 406
...
@patch.object(ApplicationProfileManager, "get_template_folder")
@patch.object(ApplicationProfileManager, "get_queries_dict")
def test_build_report_unprocessable_entity(
    mock_get_queries_dict,
    mock_get_template_folder,
    mock_dataset_description,
    mock_find_dataset_name_by_id,
    ...
    assert "Check valid application profiles and their template types through the API" in str(
        e.value
    )

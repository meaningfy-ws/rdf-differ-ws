...
def test_fuseki_diff_adapter_sparql_endpoint():
    ...
    assert (
        fuseki_service.make_sparql_endpoint(dataset_name="subdiv")
        == "http://localhost:3030/subdiv/sparql"
    )
    assert (
        fuseki_service.make_sparql_endpoint(dataset_name="/foe")
        == "http://localhost:3030/foe/sparql"
    )
...
def test_fuseki_diff_adapter_list_datasets_failing(fake_requests):
    ...
    fuseki_service = helper_fuseki_service(
        triplestore_service_url="http://some.url", http_client=fake_requests
    )
...
def test_fuseki_diff_adapter_dataset_description(fake_sparql_runner):
    ...
    assert (
        response["version_history_graph"]
        == "http://publications.europa.eu/resource/authority/subdivision/version"
    )
    assert (
        response["current_version_graph"]
        == "http://publications.europa.eu/resource/authority/subdivision/version/v2"
    )
    assert (
        "20171213-0" in response["dataset_versions"]
        and "20190220-0" in response["dataset_versions"]
    )
    assert (
        "http://publications.europa.eu/resource/authority/subdivision/version/v1"
        in response["version_named_graphs"]
    )
    assert (
        "http://publications.europa.eu/resource/authority/subdivision/version/v2"
        in response["version_named_graphs"]
...
def test_fuseki_diff_adapter_count_inserted_triples_success(fake_sparql_runner):
    fake_sparql_runner.return_value = {
        "head": {"vars": ["insertionsGraph", "triplesInInsertionGraph"]},
        "results": {
            "bindings": [
                {
                    "insertionsGraph": {
...
def test_fuseki_diff_adapter_count_deleted_triples_success(fake_sparql_runner):
    fake_sparql_runner.return_value = {
        "head": {"vars": ["deletionsGraph", "triplesInDeletionGraph", "versionGraph"]},
        "results": {
            "bindings": [
                {
                    "deletionsGraph": {

...
def test_build_report_name():
    ...
    report_location = build_report_name(
        destination_folder, dataset_name, application_profile, template_type, timestamp, extension
    )
...
def test_retrieve_report(tmpdir):
    ...
    expected_result = (
        db_location.mkdir(dataset_name)
        .mkdir(application_profile)
        .mkdir(template_type)
        .join("report.html")
    )
...
def test_save_report(tmpdir):
    ...
    assert (
        Path(db_location)
        / f"dataset/application/template_type/{dataset_name}-{application_profile}-{template_type}-{timestamp}.html"
    ).read_text() == "data"
...
def test_remove_report_success(tmpdir):
    ...
    db_location = tmpdir.mkdir("db")
    report_location = (
        db_location.mkdir(dataset_name).mkdir(application_profile).mkdir(template_type)
    )
    report_location.join("report.html")
